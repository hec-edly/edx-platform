...
@skip_unless_lms
@ddt.ddt
class MFEAppContextViewTest(ThirdPartyAuthTestMixin, APITestCase):
    """
    Mfe app auth context tests
    """
    ...
    def setUp(self):  # pylint: disable=arguments-differ
        ...
        self.url = reverse('mfe_app_auth_context')
        self.query_params = {'next': '/dashboard'}
    ...
    def get_context(self, params=None, current_provider=None, backend_name=None, add_user_details=False):
        """
        Returns the mfe app auth context
        """
        return {
            'currentProvider': current_provider,
            'platformName': settings.PLATFORM_NAME,
    ...
    def test_no_third_party_auth_providers(self):
        """
        Test that if mfe app auth is enabled, context returned by API contains
        the provider information
        """
        response = self.client.get(self.url, self.query_params)
        assert response.status_code == 200
    ...
    def test_user_country_code(self):
        """
        Test that api returns country code of user
        """
        response = self.client.get(self.url, self.query_params)
